...
(* Non deterministic since left side is same but right side different *)
...
thm WhileE
inductive_cases OrE[elim!]: "(c\<^sub>1 OR c\<^sub>2, s) \<rightarrow> ct"
thm OrE
  

(* Lemma 7.11 used to say that IMP is deterministic. 
   Once you add non-deterministic choice it no longer is.
...
next
  case (OrLeft c\<^sub>1 s t c\<^sub>2)
  show ?case using OrLeft.IH by (blast intro: star_trans)
next
  case (OrRight c\<^sub>2 s t c\<^sub>1)
  show ?case using OrRight.IH by (blast intro: star_trans)
qed
