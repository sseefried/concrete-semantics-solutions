...
lemma OrE[elim!]:
  "(c\<^sub>1 OR c\<^sub>2, s) \<Rightarrow> t \<Longrightarrow> (c\<^sub>1, s) \<Rightarrow> t \<or> (c\<^sub>2, s) \<Rightarrow> t" 
  by (erule big_step.cases, auto)
...
inductive_cases OrE2[elim!]: "(c\<^sub>1 OR c\<^sub>2, s) \<Rightarrow> t"
...
(* Lemma 7.9 IMP is deterministic can no longer be proved since it is not deterministic!*)

lemma or_comm: "c\<^sub>1 OR c\<^sub>2 \<sim> c\<^sub>2 OR c\<^sub>1" 
  by blast
